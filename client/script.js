...
*/
 ...
 sayHelloButton.addEventListener("mouseover", changeButtonBackgroundColor)
...
*/
 ...
 const changeButtonBackgroundColorBack = () => {

    sayHelloButton.style.backgroundColor = "#EFEFEF";
    sayHelloButton.style.color = "black";
 }

 sayHelloButton.addEventListener("mouseover", changeButtonBackgroundColorBack);
...
const getRequest = () => {
    ...
    axios.get(`http://localhost:3000/query-test/?name=Carlie&email=test`)
        .then(response => {
            console.log(response.data)
        })
        .catch(error => console.log(error))
...
// console logged no querys sent.

    2: Send more than 1 query on the URL -- what happened? 
    // said I sent more than one query, then I clicked the arrow and it showed both the querys that I added. 
